import PizZip from 'pizzip';

/**
 * 页眉浮动元素兜底渲染。
 *
 * docx-preview 对 Word 页眉中的浮动对象（wrapNone / behindDoc 图片、wps:wsp 文本框）
 * 支持不完整，导致 Logo、公司抬头丢失或错位。本模块解析 word/headerN.xml 中的
 * wp:anchor，把需要的浮动元素以 absolute 子元素注入到 docx-preview 渲染出的
 * section.docx 顶部，并尽量按 OOXML 语义定位。
 *
 * 当前策略：
 * - 图片：只注入 behindDoc="1" 的图片（作为背景/水印类 Logo）。如果 docx-preview
 *   已经在 <header> 中渲染了同名图片，则跳过注入，避免重复与错位。
 * - 文本框：wps:wsp 默认不渲染。如果 docx-preview 已经通过 VML/SVG 渲染出相同文本，
 *   则跳过注入，避免重影。
 * - behindDoc="1" 的元素用正值低 z-index（1），避免被白色页面背景盖住。
 */

const EMU_PER_PX = 9525;

// 常用命名空间
const NS = {
  wp: 'http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing',
  a: 'http://schemas.openxmlformats.org/drawingml/2006/main',
  pic: 'http://schemas.openxmlformats.org/drawingml/2006/picture',
  wps: 'http://schemas.microsoft.com/office/word/2010/wordprocessingShape',
  wpg: 'http://schemas.microsoft.com/office/word/2010/wordprocessingGroup',
  w: 'http://schemas.openxmlformats.org/wordprocessingml/2006/main',
  r: 'http://schemas.openxmlformats.org/officeDocument/2006/relationships',
};

interface TextBoxPadding {
  left: number;
  top: number;
  right: number;
  bottom: number;
}

interface TextBoxParagraph {
  text: string;
  fontSizePt?: number;
  bold?: boolean;
}

interface FallbackItem {
  type: 'image' | 'textbox';
  width: number; // px
  height: number; // px
  zIndex: number;
  relativeH: string;
  relativeV: string;
  offsetH: number; // EMU
  offsetV: number; // EMU
  // image
  src?: string;
  // textbox
  text?: string;
  paragraphs?: TextBoxParagraph[];
  padding?: TextBoxPadding;
  fontSizePt?: number;
}

interface SectionContext {
  section: HTMLElement;
  paddingLeft: number;
  paddingTop: number;
  scale: number;
}

/**
 * 对外主入口：读取 docxBlob 中的页眉，向 root 内的每个 section.docx 注入 fallback。
 */
export interface InjectOptions {
  /** 调试模式：给注入元素加半透明边框，便于目视定位 */
  debugHighlight?: boolean;
}

/** 测试导出：解析 docx 中所有 header 的 anchor 信息 */
export async function __test_parseHeader(docxBlob: Blob): Promise<Array<Partial<FallbackItem> & { path: string }>> {
  const buf = await docxBlob.arrayBuffer();
  const zip = new PizZip(buf);
  const headerPaths = Object.keys(zip.files).filter((p) => /^word\/header\d+\.xml$/i.test(p));
  const out: Array<Partial<FallbackItem> & { path: string }> = [];
  for (const path of headerPaths) {
    const items = parseHeader(zip, path);
    for (const it of items) {
      out.push({ ...it, path });
    }
  }
  return out;
}

export async function injectHeaderFallback(
  docxBlob: Blob,
  root: HTMLElement,
  options: InjectOptions = {}
): Promise<void> {
  const { debugHighlight = false } = options;
  const buf = await docxBlob.arrayBuffer();
  const zip = new PizZip(buf);

  // 1) 注入页眉浮动元素兜底（Logo / 文本框）。即使文档没有 header anchor，
  //    也可能存在正文留白需要处理，因此不能提前 return。
  const headerPaths = Object.keys(zip.files).filter((p) =>
    /^word\/header\d+\.xml$/i.test(p)
  );

  if (headerPaths.length > 0) {
    const headers = headerPaths
      .map((p) => parseHeader(zip, p))
      .filter((h): h is FallbackItem[] => h.length > 0);
    if (headers.length > 0) {
      const sections = Array.from(root.querySelectorAll('section.docx')) as HTMLElement[];
      sections.forEach((section, idx) => {
        const items = headers[Math.min(idx, headers.length - 1)];
        const ctx = buildSectionContext(section, root);
        injectIntoSection(section, ctx, items, options.debugHighlight);
      });
    }
  }

  // 2) 后处理：把含"以下空白"的表格行撑满 section 剩余空间，使页面在打印时与 Word
  // 的"单页填满"效果一致。docx-preview 对空段落高度的渲染通常比 Word 小，
  // 导致签名/备注区域上方出现大片空白。
  expandPlaceholderRows(root);
}

// ---------- 解压与解析 ----------

function parseHeader(zip: PizZip, headerPath: string): FallbackItem[] {
  const xmlText = zip.file(headerPath)?.asText();
  if (!xmlText) return [];

  const doc = new DOMParser().parseFromString(xmlText, 'application/xml');
  const anchors = Array.from(doc.getElementsByTagNameNS(NS.wp, 'anchor'));
  if (anchors.length === 0) return [];

  const rels = loadRels(zip, headerPath);
  const items: FallbackItem[] = [];

  for (const anchor of anchors) {
    const item = parseAnchor(anchor, zip, rels);
    if (item) items.push(item);
  }
  return items;
}

function loadRels(zip: PizZip, headerPath: string): Record<string, string> {
  const relsPath = `word/_rels/${headerPath.replace('word/', '')}.rels`;
  const text = zip.file(relsPath)?.asText();
  if (!text) return {};
  const doc = new DOMParser().parseFromString(text, 'application/xml');
  const map: Record<string, string> = {};
  for (const rel of Array.from(doc.getElementsByTagName('Relationship'))) {
    const id = rel.getAttribute('Id');
    const target = rel.getAttribute('Target');
    if (id && target) map[id] = target.replace(/^\//, '');
  }
  return map;
}

function parseAnchor(
  anchor: Element,
  zip: PizZip,
  rels: Record<string, string>
): FallbackItem | null {
  const behindDoc = anchor.getAttribute('behindDoc') === '1';

  // extent 尺寸
  const extent = anchor.getElementsByTagNameNS(NS.wp, 'extent')[0];
  const cx = parseEmu(extent?.getAttribute('cx'));
  const cy = parseEmu(extent?.getAttribute('cy'));
  if (!cx || !cy) return null;

  // 定位
  const pos = parsePosition(anchor);

  // 图片
  const blip = anchor.getElementsByTagNameNS(NS.a, 'blip')[0];
  if (blip) {
    // 只兜底 behindDoc=1 的图片；前景图 docx-preview 通常能正确渲染
    if (!behindDoc) return null;
    const embed = blip.getAttributeNS(NS.r, 'embed');
    if (!embed) return null;
    const target = rels[embed];
    if (!target) return null;
    const mediaPath = `word/${target.replace(/^word\//, '')}`;
    const file = zip.file(mediaPath);
    if (!file) return null;
    const src = dataUrlFromFile(file, mediaPath);
    if (!src) return null;
    return {
      type: 'image',
      ...pos,
      width: cx / EMU_PER_PX,
      height: cy / EMU_PER_PX,
      zIndex: behindDoc ? 1 : 10,
      src,
    };
  }

  // 文本框 wps:wsp
  const wsp = anchor.getElementsByTagNameNS(NS.wps, 'wsp')[0];
  if (wsp) {
    const paragraphs = collectParagraphs(wsp);
    const text = paragraphs.map((p) => p.text).join('\n');
    if (!text.trim()) return null;
    const padding = parseBodyPr(wsp);
    const firstFontSize = paragraphs[0]?.fontSizePt;
    return {
      type: 'textbox',
      ...pos,
      width: cx / EMU_PER_PX,
      height: cy / EMU_PER_PX,
      zIndex: behindDoc ? 1 : 10,
      text,
      paragraphs,
      padding,
      fontSizePt: firstFontSize,
    };
  }

  return null;
}

function parseEmu(v: string | null): number {
  const n = parseInt(v || '0', 10);
  return Number.isFinite(n) ? n : 0;
}

function parsePosition(anchor: Element) {
  const posH = anchor.getElementsByTagNameNS(NS.wp, 'positionH')[0];
  const posV = anchor.getElementsByTagNameNS(NS.wp, 'positionV')[0];

  const relH = posH?.getAttribute('relativeFrom') || 'column';
  const relV = posV?.getAttribute('relativeFrom') || 'paragraph';

  const offsetH = parseEmu(posH?.getElementsByTagNameNS(NS.wp, 'posOffset')[0]?.textContent || '0');
  const offsetV = parseEmu(posV?.getElementsByTagNameNS(NS.wp, 'posOffset')[0]?.textContent || '0');

  return {
    relativeH: relH,
    relativeV: relV,
    offsetH,
    offsetV,
  };
}

function dataUrlFromFile(file: any, path: string): string | null {
  try {
    const ext = path.split('.').pop()?.toLowerCase() || 'png';
    const mime =
      ext === 'png'
        ? 'image/png'
        : ext === 'jpg' || ext === 'jpeg'
        ? 'image/jpeg'
        : ext === 'gif'
        ? 'image/gif'
        : ext === 'svg'
        ? 'image/svg+xml'
        : 'image/png';

    let b64 = '';
    if (typeof file.asBase64 === 'function') {
      b64 = file.asBase64();
    } else if (typeof file.asUint8Array === 'function') {
      const arr = file.asUint8Array() as Uint8Array;
      b64 = uint8ToBase64(arr);
    } else if (typeof file.asArrayBuffer === 'function') {
      b64 = uint8ToBase64(new Uint8Array(file.asArrayBuffer()));
    } else if (typeof file.asBinary === 'function') {
      b64 = btoa(file.asBinary());
    }
    if (!b64) return null;
    return `data:${mime};base64,${b64}`;
  } catch {
    return null;
  }
}

function uint8ToBase64(arr: Uint8Array): string {
  const chunk = 0x8000;
  let parts: string[] = [];
  for (let i = 0; i < arr.length; i += chunk) {
    parts.push(String.fromCharCode.apply(null, Array.from(arr.subarray(i, i + chunk))));
  }
  return btoa(parts.join(''));
}

function isInsideFallback(el: Element): boolean {
  let node: Element | null = el;
  while (node) {
    if (node.tagName === 'mc:Fallback') return true;
    node = node.parentElement;
  }
  return false;
}

function collectParagraphs(el: Element): TextBoxParagraph[] {
  // 优先取 wps:txbx 里的段落；若不存在则取整个元素
  const txbx = el.getElementsByTagNameNS(NS.wps, 'txbx')[0] || el;
  const pEls = Array.from(txbx.getElementsByTagNameNS(NS.w, 'p'));
  const paragraphs: TextBoxParagraph[] = [];
  for (const p of pEls) {
    // 跳过 Word 兼容性 VML fallback 中的重复文本
    if (isInsideFallback(p)) continue;
    const texts = Array.from(p.getElementsByTagNameNS(NS.w, 't'));
    const line = texts.map((t) => t.textContent || '').join('');
    if (!line.trim()) continue;
    paragraphs.push({
      text: line,
      fontSizePt: parseParagraphFontSize(p),
      bold: !!p.getElementsByTagNameNS(NS.w, 'b')[0],
    });
  }
  return paragraphs;
}

function parseParagraphFontSize(p: Element): number | undefined {
  // 优先取 run 属性里的 sz，再取段落属性里的 sz
  const sz =
    p.getElementsByTagNameNS(NS.w, 'sz')[0] ||
    p.getElementsByTagNameNS(NS.w, 'rPr')[0]?.getElementsByTagNameNS(NS.w, 'sz')[0];
  if (!sz) return undefined;
  const v = parseInt(sz.getAttribute('w:val') || sz.getAttribute('val') || '0', 10);
  return v > 0 ? v / 2 : undefined; // half-points -> points
}

function parseBodyPr(wsp: Element): TextBoxPadding {
  const bodyPr = wsp.getElementsByTagNameNS(NS.wps, 'bodyPr')[0];
  const parseIns = (attr: string) => {
    const v = bodyPr?.getAttribute(attr);
    return v ? parseInt(v, 10) / EMU_PER_PX : 0;
  };
  return {
    left: parseIns('lIns'),
    top: parseIns('tIns'),
    right: parseIns('rIns'),
    bottom: parseIns('bIns'),
  };
}

function parseFontSize(wsp: Element): number | undefined {
  const sz = wsp.getElementsByTagNameNS(NS.w, 'sz')[0];
  if (!sz) return undefined;
  const v = parseInt(sz.getAttribute('w:val') || sz.getAttribute('val') || '0', 10);
  return v > 0 ? v / 2 : undefined; // half-points -> points
}

// ---------- 注入 ----------

function buildSectionContext(section: HTMLElement, root: HTMLElement): SectionContext {
  const style = getComputedStyle(section);
  const paddingLeft = parseFloat(style.paddingLeft) || 0;
  const paddingTop = parseFloat(style.paddingTop) || 0;
  const scale = detectScale(section);
  return { section, paddingLeft, paddingTop, scale };
}

function detectScale(el: HTMLElement): number {
  const rect = el.getBoundingClientRect();
  const layoutW = el.offsetWidth;
  if (layoutW && rect.width) return rect.width / layoutW;
  const layoutH = el.offsetHeight;
  if (layoutH && rect.height) return rect.height / layoutH;
  return 1;
}

function injectIntoSection(
  section: HTMLElement,
  ctx: SectionContext,
  items: FallbackItem[],
  debugHighlight = false
): void {
  // 确保 section 能作为 absolute 定位上下文
  if (getComputedStyle(section).position === 'static') {
    section.style.position = 'relative';
  }

  const headerRoot = section.querySelector('header') as HTMLElement | null;

  const layer = document.createElement('div');
  layer.className = 'docx-header-fallback';
  layer.style.cssText =
    'position:absolute;left:0;top:0;width:100%;height:100%;pointer-events:none;overflow:visible;z-index:1;';

  const injectedImageSrcs = new Set<string>();

  // 测量 header 内第一个段落的实际顶部位置（用于 relativeFrom="paragraph"/"line"）
  // 在 docx-preview 中，section 是 flex column，header 被 margin-top 拉起，
  // 段落的实际 top 在 section 局部坐标里 ≈ paddingTop + header.marginTop。
  // 不依赖测量的话会与原生 docx-preview 渲染差 50+ px。
  // getBoundingClientRect 返回 viewport px，需除以 scale 转回 layout px（与 EMU 换算同基准）。
  const anchorParagraph = headerRoot ? (headerRoot.querySelector('p') as HTMLElement | null) : null;
  const paragraphTopInSection = anchorParagraph
    ? (anchorParagraph.getBoundingClientRect().top - section.getBoundingClientRect().top) / ctx.scale
    : ctx.paddingTop; // fallback: 假设段落顶在 padding edge

  // 第一步：先按 OOXML 语义算出每个元素的原始位置
  const placed: { item: FallbackItem; pos: { left: number; top: number } }[] = [];
  for (const item of items) {
    const pos = resolvePosition(item, ctx, paragraphTopInSection);
    if (pos) placed.push({ item, pos });
  }

  // 第二步：保守处理——页眉浮动元素不应压到正文区域。
  // 若某些元素底部超出正文起始位置（paddingTop），则把所有元素整体向上平移
  // 最大溢出量，保持 Logo 与文本框等元素的相对位置不变。
  const overflow = Math.max(
    0,
    ...placed.map(({ item, pos }) => pos.top + item.height - ctx.paddingTop)
  );
  if (overflow > 0) {
    for (const { pos } of placed) {
      pos.top -= overflow;
    }
  }

  for (const { item, pos } of placed) {
    if (item.type === 'image' && item.src) {
      const img = document.createElement('img');
      img.src = item.src;
      img.dataset.fallbackType = 'image';
      img.style.cssText =
        `position:absolute;left:${pos.left}px;top:${pos.top}px;` +
        `width:${item.width}px;height:${item.height}px;` +
        `z-index:${item.zIndex};pointer-events:none;` +
        (debugHighlight ? `border:2px dashed rgba(255,0,0,0.6);background:rgba(255,0,0,0.08);` : '');
      layer.appendChild(img);
      injectedImageSrcs.add(item.src);
    } else if (item.type === 'textbox') {
      const paragraphs = item.paragraphs && item.paragraphs.length > 0 ? item.paragraphs : [{ text: item.text || '' }];
      const div = document.createElement('div');
      const pad = item.padding || { left: 0, top: 0, right: 0, bottom: 0 };
      div.dataset.fallbackType = 'textbox';
      // 显式指定字体，避免浏览器用中文字体（粗且宽）渲染英文导致换行
      // Word 文档默认 ascii=Times New Roman / eastAsia=宋体
      const baseFont = '"Times New Roman","宋体",serif';
      div.style.cssText =
        `position:absolute;left:${pos.left}px;top:${pos.top}px;` +
        `width:${item.width}px;height:${item.height}px;` +
        `box-sizing:border-box;` +
        `padding:${pad.top}px ${pad.right}px ${pad.bottom}px ${pad.left}px;` +
        `z-index:${item.zIndex};pointer-events:none;` +
        `font-family:${baseFont};` +
        `display:flex;flex-direction:column;justify-content:flex-start;align-items:center;text-align:center;overflow:visible;` +
        (debugHighlight ? `border:2px dashed rgba(0,128,255,0.6);background:rgba(0,128,255,0.08);` : '');
      for (const p of paragraphs) {
        const pDiv = document.createElement('div');
        // 不强制 word-break，靠 overflow:visible 让英文在一行内完整显示
        pDiv.style.cssText = 'white-space:pre;width:100%;overflow:visible;';
        if (p.fontSizePt) {
          pDiv.style.fontSize = `${p.fontSizePt}pt`;
        }
        if (p.bold) {
          pDiv.style.fontWeight = 'bold';
        }
        // 显式中英文字体
        pDiv.style.fontFamily = baseFont;
        pDiv.textContent = p.text;
        div.appendChild(pDiv);
      }
      layer.appendChild(div);
    }
  }

  if (layer.childNodes.length === 0) return;

  // 隐藏 docx-preview 原生渲染到 <header> 中的浮动对象，避免与 fallback 重影/错位
  hideNativeHeaderFloats(headerRoot);

  section.appendChild(layer);

  if (injectedImageSrcs.size > 0) {
    suppressDuplicateHeaderImages(section, injectedImageSrcs);
  }
}

function isTextBoxRendered(headerText: string, text: string): boolean {
  const fingerprint = text.trim().replace(/\s+/g, '').slice(0, 20);
  if (!fingerprint) return false;
  return headerText.includes(fingerprint);
}

function resolvePosition(
  item: FallbackItem,
  ctx: SectionContext,
  paragraphTopInSection: number
): { left: number; top: number } | null {
  // OOXML 定位语义（section 局部坐标，以 section 边框左上角为原点）：
  //  - column：在单列布局下 = 内容区左上角（section 的 padding edge），
  //           因此水平基准 = paddingLeft，垂直基准 = paddingTop。
  //  - page / margin：= section 外边（左/上 = 0）。
  //  - paragraph / line：= 锚定段落的实际顶部，由 injectIntoSection 实测传入。
  // 注意：fallback 层是 section 的 absolute 子元素，CSS left/top 以 section padding edge 为
  // 原点（即 section 局部坐标），所以此处算出的坐标可直接赋给 style.left/top。
  const offsetH = item.offsetH / EMU_PER_PX;
  const offsetV = item.offsetV / EMU_PER_PX;

  let baseH: number;
  if (item.relativeH === 'page' || item.relativeH === 'margin') {
    baseH = 0;
  } else {
    // column：相对内容区左边缘 = section 的 paddingLeft
    baseH = ctx.paddingLeft;
  }

  let baseV: number;
  if (item.relativeV === 'page' || item.relativeV === 'margin') {
    baseV = 0;
  } else if (item.relativeV === 'paragraph' || item.relativeV === 'line') {
    baseV = paragraphTopInSection;
  } else {
    // column 默认当作 padding edge
    baseV = ctx.paddingTop;
  }

  return {
    left: baseH + offsetH,
    top: baseV + offsetV,
  };
}

function hideNativeHeaderFloats(headerRoot: HTMLElement | null): void {
  if (!headerRoot) return;
  // docx-preview 会把 floating 文本框渲染成绝对定位 SVG，把 floating 图片渲染成 img。
  // 这些原生元素位置经常错误，直接隐藏，由 fallback 完全接管。
  const selectors = [
    'svg',
    'img',
    'foreignObject',
    '.vml-shape',
    '[style*="position:absolute"]',
    '[style*="position: absolute"]',
  ];
  for (const sel of selectors) {
    for (const el of Array.from(headerRoot.querySelectorAll(sel))) {
      const htmlEl = el as HTMLElement;
      try {
        htmlEl.style.display = 'none';
      } catch {
        // ignore
      }
    }
  }
}

function suppressDuplicateHeaderImages(
  section: HTMLElement,
  srcSet: Set<string>
): void {
  // docx-preview 原生的 behindDoc=1 图片可能被渲染到 <header> 中并错位，
  // 用 src 匹配隐藏同名图片。
  const imgs = Array.from(section.querySelectorAll('header img, .docx-header img')) as HTMLImageElement[];
  for (const img of imgs) {
    try {
      if (srcSet.has(img.src)) {
        img.style.display = 'none';
      }
    } catch {
      // ignore cross-origin/style access issues
    }
  }
}

// ---------- 正文表格后处理 ----------

/**
 * 对含"以下空白"占位文本的表格行进行撑满处理。
 *
 * Word 模板常在该行写入多个空段落以撑满 A4 页面；docx-preview 对空段落
 * 高度的渲染通常不足，导致打印时页面下方留白。本函数在渲染后检测这类行，
 * 将其单元格高度增加到接近填满 section 的可用正文区域，同时保留安全边距，
 * 避免某些浏览器/打印机的默认边距导致内容被推到第二页。
 */
function expandPlaceholderRows(root: HTMLElement): void {
  const sections = Array.from(root.querySelectorAll('section.docx')) as HTMLElement[];
  for (const section of sections) {
    const rows = Array.from(section.querySelectorAll('table tr')) as HTMLTableRowElement[];
    const fillRows = rows.filter((tr) => /以下空白/.test((tr.textContent || '').trim()));
    if (fillRows.length === 0) continue;

    const article = section.querySelector('article') as HTMLElement | null;
    if (!article) continue;

    const style = getComputedStyle(section);
    const padTop = parseFloat(style.paddingTop) || 0;
    const padBottom = parseFloat(style.paddingBottom) || 0;
    const available = section.offsetHeight - padTop - padBottom;

    // 预留 ~5% 安全边距：不同浏览器/打印机的默认边距、页眉页脚高度会有差异，
    // 把 article 撑到可用区域的 95% 而不是 100%，在"接近填满"和"避免分页"之间取平衡。
    const target = Math.floor(available * 0.95);
    const diff = target - article.offsetHeight;
    if (diff <= 5) continue;

    // 多个占位行时均摊增量，避免单行过高被浏览器强制分页。
    const addPerRow = Math.floor(diff / fillRows.length);

    for (const fillRow of fillRows) {
      const cell = fillRow.querySelector('td, th') as HTMLElement | null;
      if (!cell) continue;

      // 解除 Word 模板可能设置的固定行高限制。
      fillRow.style.height = 'auto';
      cell.style.height = 'auto';

      // 同时设置 height 与 minHeight，确保 table 布局最终把行高撑到目标值。
      const targetHeight = cell.offsetHeight + addPerRow;
      cell.style.height = `${targetHeight}px`;
      cell.style.minHeight = `${targetHeight}px`;
      cell.style.boxSizing = 'border-box';
    }
  }
}
